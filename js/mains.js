...
$('.js-imageChangeBtn').on('click',function(){
  var $this = $(this),
   ...
   setTimeout(() => {
        ...
        $(secondImgId).attr('src', secondImg).addClass('fadeInUp');
        $(thirdImgId).attr('src', thirdImg).addClass('fadeInUp');
   },750);
...
//function CatMenuBorderAnimation() {
//  var nav = $(document).find(".sub-category-nav-list"),
//    navLine = $(document).find(".js-cat-nav-line"),
//    offsetLeft = nav.offset().left,
//    currentMenu = nav.find("li.current-cat-nav-item > a"),
//    currentMenuOffset = currentMenu.offset(),
//    currentMenuLeft = currentMenuOffset
//      ? currentMenuOffset.left - offsetLeft
//      : 0,
...
function imageHotSpot($this){
  var overlayDiv = $('.overlay-text'),
    ...
    TweenLite.to(overlayDiv, 0.5, {css: {left:_left, top:_top},ease: Circ.easeOut});   
